...
#             self.conv3.bias.data.fill_(0)
        ...
        self.fc_mu = nn.Linear(32, 4, bias = False)
        self.fc_logvar = nn.Linear(32, 4, bias = False) 
...
#             self.fc_transcode_d2.bias.data.fill_(0)'''
        ...
        for i in range(LATENT_DIM):
            self.fc_transcode[i] = nn.Linear(2, 256, bias = True)
            
        # identity (from third and fourth latent state)
        self.fc_gate = [0] * LATENT_DIM
        for i in range(LATENT_DIM):
            self.fc_gate[i] = nn.Linear(2, 1, bias = True)
...
#             self.deconv2.bias.data.fill_(0)
    ...
    def forward(self, x, z, u_in):  
        ...
        gate_in = u_out[:,2:].detach().view(-1, 2)
        for i in range(LATENT_DIM):
            gate = F.relu(self.fc_gate[i](gate_in)).expand_as(x_hat)
            x_hat += F.relu(self.fc_transcode[i](trans)) * gate
            '''if i == 0:
