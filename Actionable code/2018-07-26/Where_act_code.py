...
class Net(nn.Module):
    def __init__(self, args):
        ...
        self.fc_label_d1 = nn.Linear(NB_LABEL, LATENT_DIM, bias = BIAS_LABEL)
        self.fc_label_d2 = nn.Linear(NB_LABEL, LATENT_DIM, bias = BIAS_LABEL)
        if BIAS_LABEL == True:
            self.fc_label_d1.bias.data.fill_(0)
            self.fc_label_d2.bias.data.fill_(0)
...
#             self.deconv2.bias.data.fill_(0)
    ...
    def forward(self, x, z, u_in):  
        ...
        # Category input (Linear)
        z_d1 = self.fc_label_d1(z)
        z_d2 = self.fc_label_d2(z)
        
        # translation input (transformation)
        u_d1 = u_in[:,0].view(-1, 1) * 2 / size  #torch.cat((u[:,0],u[:,0],u[:,0],u[:,0],u[:,0],u[:,0],u[:,0],u[:,0]), 1)
        u_d2 = u_in[:,1].view(-1, 1) * 2 / size
        for _ in range(5):
        ...
        return x_hat_logit, transfo_d1, transfo_d2  
...
class ML():
    ...
    def train_epoch(self, epoch, rank=0):
        ...
        for batch_idx, (data, target) in enumerate(self.dataset.data_loader):
            ...
            x_hat_logit_output = self.model(data_full, label_full, pos_full)[0]
            ...
            if self.args.verbose and self.args.log_interval>0:
                if batch_idx % self.args.log_interval == 0:
                    print('\tTrain Epoch: {} [{}/{} ({:.0f}%)]\tBCELoss: {:.6f}'.format(
