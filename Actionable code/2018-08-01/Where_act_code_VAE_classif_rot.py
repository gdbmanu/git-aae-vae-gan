...
#             self.conv3.bias.data.fill_(0)
        ...
        self.fc_mu = nn.Linear(32, DIM_U + DIM_Z, bias = False)
        self.fc_logvar = nn.Linear(32, DIM_U + DIM_Z, bias = False) 
        
        # classif output path (from third latent state)
        self.fc_classif_1 = nn.Linear(DIM_Z, 256, bias = True)
        self.fc_classif_2 = nn.Linear(256, NB_LABEL, bias = True)
        
        # transcoding from feature space to visual space
        '''self.fc_z_d1 = nn.Linear(3, LATENT_DIM, bias = True)
        self.fc_z_d2 = nn.Linear(3, LATENT_DIM, bias = True)
        self.fc_z_d3 = nn.Linear(3, LATENT_DIM, bias = True)'''
        # !! gate !!
...
#             self.fc_transcode_d2.bias.data.fill_(0)'''
        ...
        self.z_bias = torch.randn(LATENT_DIM, 2)
        self.fc_zd = [0] * LATENT_DIM
        self.fc_transcode = [0] * LATENT_DIM
        for i in range(LATENT_DIM):
            self.fc_zd[i] = nn.Linear(DIM_Z, 3, bias = True)
            self.fc_transcode[i] = nn.Linear(3, 256, bias = True)
...
#             self.deconv2.bias.data.fill_(0)
    ...
    def forward(self, x, z, u_in):  
        ...
        z_latent = u_out[:,DIM_U:].view(-1, DIM_Z) 
        ...
        z_d1 = self.fc_z_d1(u_out[:,3:].view(-1, 3).detach())
        #z_d1 = 
        #z_d2 = F.relu(self.fc_z_d2(z_latent.detach()))
        z_d2 = self.fc_z_d2(u_out[:,3:].view(-1, 3).detach())
        z_d3 = self.fc_z_d3(u_out[:,3:].view(-1, 3).detach())'''
        ...
        rot_cos[0] = torch.cos(u_out[:,0]).view(-1,1,1)
        rot_sin[0] = torch.sin(u_out[:,0]).view(-1,1,1)
        rot_cos[1] = torch.cos(u_out[:,1]).view(-1,1,1)
        rot_sin[1] = torch.sin(u_out[:,1]).view(-1,1,1)
        rot_cos[2] = torch.cos(u_out[:,2]).view(-1,1,1)
        rot_sin[2] = torch.sin(u_out[:,2]).view(-1,1,1)
        rot_zero = torch.zeros_like(rot_cos[0])
        rot_un = torch.ones_like(rot_cos[0])

        rot_0_1 =  torch.cat((rot_cos[0], -rot_sin[0], rot_zero), 2)
        rot_0_2 =  torch.cat((rot_sin[0], rot_cos[0], rot_zero), 2)
        rot_0_3 =  torch.cat((rot_zero, rot_zero, rot_un), 2)
        rot[0] = torch.cat((rot_0_1, rot_0_2, rot_0_3), 1)

        rot_1_1 =  torch.cat((rot_cos[1], rot_zero, rot_sin[1]), 2)
        rot_1_2 =  torch.cat((rot_zero, rot_un, rot_zero), 2)
        rot_1_3 =  torch.cat((-rot_sin[1], rot_zero, rot_cos[1]), 2)
        rot[1] = torch.cat((rot_1_1, rot_1_2, rot_1_3), 1)

        rot_2_1 =  torch.cat((rot_un, rot_zero, rot_zero), 2)
        rot_2_2 =  torch.cat((rot_zero, rot_cos[2], -rot_sin[2]), 2)
        rot_2_3 =  torch.cat((rot_zero, rot_sin[2], rot_cos[2]), 2)
        rot[2] = torch.cat((rot_2_1, rot_2_2, rot_2_3), 1)
        
        
        for i in range(LATENT_DIM):
            h1 = u_out[:,DIM_U].detach() - self.z_bias[i,0].item()
            #h2 = u_out[:,DIM_U + 1].detach() - self.z_bias[i,1].item()
            #h3 = u_out[:,DIM_U + 2].detach() - self.z_bias[i,2].item()
            h = h1.view(-1,1) #torch.cat((h1.view(-1,1), h2.view(-1,1)), 1) #, h3.view(-1,1)), 1)
            #z_3D = h.view(-1,3,1)
            z_3D = self.fc_zd[i](h).view(-1,3,1)
            #z_3D = torch.cat((z_d1[:,i].view(-1,1), z_d2[:,i].view(-1,1), z_d3[:,i].view(-1,1)), 1).view(-1,3,1).detach()  
            #z_3D = torch.cat((z_d1[:,i].view(-1,1), z_d2[:,i].view(-1,1), z_d3[:,i].view(-1,1)), 1).view(-1,3,1).detach()  
            
            #z_3D = u_out[:,3:].detach().view(-1, 3, 1) 
            
            #transformed_z = torch.matmul(hom, torch.matmul(rot, z_2D)) +  trans
            transformed_z = torch.matmul(rot[0], z_3D)
            transformed_z = torch.matmul(rot[1], transformed_z)
            #transformed_z = torch.matmul(rot[2], transformed_z)           
            transformed_z = transformed_z.view(-1,3)
        ...
        return  x_hat_logit, mu, logvar, u_out, z_hat_logit, transfo_d1, transfo_d2, transfo_d3 
        ...
        for batch_idx, (data, target) in enumerate(self.dataset.data_loader):
            ...
            if self.args.verbose and self.args.log_interval>0:
                if batch_idx % self.args.log_interval == 0:
                    print('Train Epoch: {} [{}/{} ({:.0f}%)]\tclassif Loss: {:.2f}\tBCE_loss: {:.2f}\tKL: {:.2f}\tAcc: {:.2f}'.format(
