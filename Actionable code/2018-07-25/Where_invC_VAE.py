...
class Net(nn.Module):
    def __init__(self, args):
        ...
        self.fc_label = nn.Linear(NB_LABEL, LATENT_DIM, bias = BIAS_LABEL)
        if BIAS_LABEL == True:
            self.fc_label.bias.data.fill_(0)
            
        self.fc_x = nn.Linear(LATENT_DIM, 32, bias = False)
        self.fc_z = nn.Linear(LATENT_DIM, 32, bias = False)             
        # Mu encoder
        ...
        self.fc_pos_2 = nn.Linear(32, LATENT_DIM, bias = BIAS_POS)
        if BIAS_POS == True:
            self.fc_pos_1.bias.data.fill_(0)    
            self.fc_pos_2.bias.data.fill_(0)    
         
        self.deconv4 = nn.ConvTranspose2d(1024, 256, 4, bias = BIAS_DECONV, stride=4, padding=0)
    ...
    def forward(self, x, z, u_in):  
        ...
        # Merge
        merge_1 = F.relu(self.fc_x(F.relu(x)) + self.fc_z(F.relu(z)))
        ...
        u = F.relu(self.fc_pos_1(u_out))
        u = self.fc_pos_2(u)
        
        # Merge image and position information
        merge_2 = F.relu(z + u) # 
        x_hat = merge_2.view(-1, LATENT_DIM, 1, 1)
        ...
        return mu, logvar, u_out, x_hat_logit #F.softmax(z, dim = 1)    
...
class ML():
    ...
    def train_epoch(self, epoch, rank=0):
        ...
        for batch_idx, (data, target) in enumerate(self.dataset.data_loader):
            ...
            mu_output, logvar_output, u_output, x_hat_logit_output = self.model(data_full, label_full, pos_full)

            # print(output.shape, acc_full.shape)
            # Compute the loss based on the predictions and actual labels
            loss_x_hat = self.loss_func(x_hat_logit_output, data_full)
